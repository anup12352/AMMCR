...
// but with calculated group velocity from band structure and DOS both calculated from DFT:
{
    ...
    if (T < T_trans)
    {
        ...
        for (int counter = 0;counter<points-1;counter++)
        {
            ...
            {
                g_LO[counter] = (-1)*e*E/(h_bar*nu_el[counter] * (1 + beta1[counter] * beta1[counter])) * 						(df0dk(k_grid,k_grid[counter],T,E_f,coefficients,kindex,a)+i*ddf)*6.241509324e11;

                h_LO[counter] = (e*E*beta1[counter])/(h_bar*nu_el[counter] * (1 + beta1[counter] * beta1[counter])) * 						(df0dk(k_grid,k_grid[counter],T,E_f,coefficients,kindex,a)+i*ddf)*6.241509324e11;
	     }	

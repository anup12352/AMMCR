...
{   
	...
	*/
	...
	for(int i=0;i<number;i++)
	{
		...
		h(0,0) = Es0 + 8*Vsss*(cos(kx/2.0)*cos(ky/2.0)*cos(kz/2.0));
		h(1,1) = Ep0 + ((8.0/3.0)*Vpps+(16.0/3.0)*Vppp)*(cos(kx/2.0)*cos(ky/2.0)*cos(kz/2.0));
		h(2,2) = Ep0 + ((8.0/3.0)*Vpps+(16.0/3.0)*Vppp)*(cos(kx/2.0)*cos(ky/2.0)*cos(kz/2.0));
		h(3,3) = Ep0 + ((8.0/3.0)*Vpps+(16.0/3.0)*Vppp)*(cos(kx/2.0)*cos(ky/2.0)*cos(kz/2.0));
		
		
		h(0,1) = (8.0/sqrt(3))*ii*Vsps*(sin(kx/2.0)*cos(ky/2.0)*cos(kz/2.0));
		h(1,0) = conj(h(0,1));

		h(0,2) = (8.0/sqrt(3))*ii*Vsps*(sin(ky/2.0)*cos(kx/2.0)*cos(kz/2.0));
		h(2,0) = conj(h(0,2));

		h(0,3) = (8.0/sqrt(3))*ii*Vsps*(sin(kz/2.0)*cos(kx/2.0)*cos(ky/2.0));
		h(3,0) = conj(h(0,3));;

		h(1,2) = (-8.0/3.0)*(Vpps-Vppp)*sin(kx/2.0)*sin(ky/2.0)*cos(kz/2.0);
		h(2,1) =conj( h(1,2));

		h(1,3) = (-8.0/3.0)*(Vpps-Vppp)*sin(kx/2.0)*sin(kz/2.0)*cos(ky/2.0);
		h(3,1) = conj(h(1,3));
		
		h(2,3) = (-8.0/3.0)*(Vpps-Vppp)*sin(ky/2.0)*sin(kz/2.0)*cos(kx/2.0);
		h(3,2) = conj(h(2,3));
		//*/

...
{   
	...
	*/
	...
	for(int i=0;i<number;i++)
	{
		...
		h(0,0) = Es0 + 2*Vsss*(cos(kx)+cos(ky)+cos(kz));
		h(1,1) = Ep0 + 2*Vpps*cos(kx) +2*Vppp*(cos(ky)+cos(kz));
		h(2,2) = Ep0 + 2*Vpps*cos(ky) +2*Vppp*(cos(kx)+cos(kz));
		h(3,3) = Ep0 + 2*Vpps*cos(kz) +2*Vppp*(cos(kx)+cos(ky));
		
		
		h(0,1) = 2.0*ii*Vsps*sin(kx);
		h(1,0) = conj(h(0,1));
		//h(1,0) = -1*2*sqrt(2)*ii*(sin(kx/2)*cos(ky/2)+sin(kx/2)*cos(kz/2))*Vsps;

		h(0,2) = 2.0*ii*Vsps*sin(ky);
		h(2,0) = conj(h(0,2));

		h(0,3) = 2.0*ii*Vsps*sin(kz);
		h(3,0) = conj(h(0,3));;
